...
class Video3DDataset(Dataset):
	def __init__(self, video_path, train=False, img_transform=img_transform, mask_transform=mask_transform, mask_filter='CannyEdgeDetection', evaluate=False):
		...
		self.min_size = 20
		self.data_size = self.find_min_video_size()
		self.mask_transform = SampleEntropy(self.min_size-1)
	...
	def find_min_video_size(self):
		...
		for vfp in self.videos_full_path:
			...
			size = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
			no_data_pts = int(size/self.min_size)
			self.video_data_pts.append(no_data_pts)
			data_size += no_data_pts
			cap.release()
		return data_size
	...
	def __getitem__(self, index):
		...
		for i in range(len(self.video_data_pts)):
			if index - self.video_data_pts[i] > 0:
				index = index - self.video_data_pts[i]
			else:
				video_idx = i
				data_pt = index

		frame = []
		cap = cv2.VideoCapture(self.videos_full_path[video_idx])
		for dp in range(data_pt+1):
			for i in range(self.min_size):
				_, frame_i = cap.read()
				if dp == data_pt:
					frame.append(frame_i)
